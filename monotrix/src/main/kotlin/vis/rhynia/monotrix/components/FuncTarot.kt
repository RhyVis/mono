...
import vis.rhynia.monotrix.elements.data.tarot.TarotCardDrawn
import vis.rhynia.monotrix.elements.data.tarot.TarotDeck
import vis.rhynia.monotrix.elements.data.tarot.TarotDeckInfo
import vis.rhynia.monotrix.elements.web.ApiResponse
import vis.rhynia.monotrix.elements.web.json.ConfMapping
...
class FuncTarot(
    ...
    fun init() {
        ...
        for (entry in mainConf.mappings) {
            val deckFileUri =
                UriComponentsBuilder
                    .fromHttpUrl(conf.endpoint)
                    .pathSegment("tarot", "conf", entry.value)
                    .encode()
                    .build()
                    .toUriString()
    ...
    fun deckInfo(): ApiResponse = ApiResponse(deckInfo)
